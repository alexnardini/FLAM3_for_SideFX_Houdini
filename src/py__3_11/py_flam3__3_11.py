...
class flam3h_iterator_prm_names:
    ...
    Note:
        ...
        * flam3h_iterator_utils.menu_select_iterator_data(self, data_now: tuple[list[Any] | Any, ...], data_names: tuple[str, ...]) -> list:
...
* build_menu_vars_all_indexes(self) -> dict[int, int]:
    ...
    def __populate_linear_list(linear: list, item: str, id: int, spacer: bool = True) -> None:
        ...
        """
        linear.append(id)
        if spacer: linear.append(f"{item}                    ") # 20 times \s
...
* flam3h_on_deleted(self) -> None:
    ...
    def is_post_affine_default_on_load(node: hou.SopNode) -> None:
        ...
        [node.setParms({f"{prm_list_post_affine[0][0]}{id + 1}": 0}) if node.parm(f"{prm_list_post_affine[0][0]}{id + 1}").eval() and 1 not in keyframes[id] and affine == AFFINE_DEFAULT_VALS else ... for id, affine in enumerate(collect)] # type: ignore
    ...
    def hou_session_data_clear_and_restore() -> None:
        ...
        try: del hou.session.F3H_SYS_UPDATE_MODE # type: ignore
        except: pass
    ...
    def flam3h_on_create_compatible_false(self, iterators_count_zero: bool = True, descriptive_prm: bool = True) -> None:
        ...
        # Set prper messages in the about tabs
...
* reset_PREFS(self, mode: int = 0) -> None:
    ...
    def util_store_all_viewers(self) -> None:
        ...
        if _ENTER_PRM is not None and _ENTER_PRM == OUT_RENDER_PROPERTIES_SENSOR_ENTER:
            ...
            try: hou.session.F3H_SENSOR_CAM_STASH_COUNT # type: ignore
            except: hou.session.F3H_SENSOR_CAM_STASH_COUNT: int = len(views_cam) # type: ignore
            try: hou.session.F3H_SENSOR_CAM_STASH_DICT # type: ignore
            except: hou.session.F3H_SENSOR_CAM_STASH_DICT: dict[str, hou.GeometryViewportCamera] = dict(zip(views_keys, views_cam)) # type: ignore
            try: hou.session.F3H_SENSOR_CAM_STASH_TYPE_DICT # type: ignore
            except: hou.session.F3H_SENSOR_CAM_STASH_TYPE_DICT: dict[str, hou.geometryViewportType] = dict(zip(views_keys, views_type)) # type: ignore
    ...
    def flam3h_toggle_mp_xf_viz(self) -> None:
        ...
        if prm_mp.eval():
            ...
            self.private_prm_set(node, PREFS_PVT_XF_VIZ_SOLO_MP_IDX, s_mp_index)
            self.private_prm_set(node, PREFS_PVT_XF_FF_VIZ_SOLO, 0)
...
FLAM3H_ICON_COPY_PASTE_INFO_WARN = '![opdef:/alexnardini::Sop/FLAM3H?icon_optionStarWarningSVG.svg]'
...
* menu_T_pb(self) -> list:
* menu_select_iterator_data(self, data_now: tuple[list[Any] | Any, ...], data_names: tuple[str, ...]) -> list:
...
* iterator_vactive_and_update(self) -> None:
    ...
    def flam3h_init_hou_session_iterator_data(node: hou.SopNode) -> None:
        ...
        try: hou.session.F3H_MARKED_ITERATOR_NODE # type: ignore
        except: hou.session.F3H_MARKED_ITERATOR_NODE: TA_MNode = node # type: ignore
        try: hou.session.F3H_MARKED_ITERATOR_NODE.type() # type: ignore
        except: hou.session.F3H_MARKED_ITERATOR_NODE: TA_MNode = None # type: ignore
        try: hou.session.F3H_MARKED_ITERATOR_MP_IDX # type: ignore
        except: hou.session.F3H_MARKED_ITERATOR_MP_IDX: TA_M = None # type: ignore
    ...
    def flam3h_init_hou_session_ff_data(node: hou.SopNode) -> None:
        ...
        # becasue FLAM3H™ node will create and initialize those on creation
        # but just in case this data is deleted somehow.
        try: hou.session.F3H_MARKED_FF_NODE # type: ignore
        except: hou.session.F3H_MARKED_FF_NODE: TA_MNode = node # type: ignore
        try: hou.session.F3H_MARKED_FF_NODE.type() # type: ignore
        except: hou.session.F3H_MARKED_FF_NODE: TA_MNode = None # type: ignore
        try: hou.session.F3H_MARKED_FF_CHECK # type: ignore
        except: hou.session.F3H_MARKED_FF_CHECK: TA_M = None # type: ignore
    ...
    def is_FF_affine_default(node: hou.SopNode, from_FLAM3H_NODE: hou.SopNode, prm_list_affine: tuple, post: bool = False) -> bool:
        ...
        if post:
            ...
            keyframes: list = [item for sublist in [[1 if len(p.keyframes()) else 0 for p in node.parmTuple(f"{prm_list_affine[idx][0]}")] if prm_list_affine[idx][1] else [1 if len(node.parm(f"{prm_list_affine[idx][0]}").keyframes()) else 0] for idx in range(len(prm_list_affine))] for item in sublist]
    ...
    def menu_select_iterator_data(self, data_now: tuple[list[Any] | Any, ...], data_names: tuple[str, ...]) -> list:
    ...
    def menu_select_iterator(self) -> list:
        ...
        with hou.undos.disabler(): # type: ignore
            ...
            cached: list | None = node.cachedUserData('iter_sel')
                ...
                if isDELETED: return MENU_ITER_COPY_PASTE_DELETED_MARKED
                    ...
                    if from_FLAM3H_NODE is not None:
                        ...
                        if node == from_FLAM3H_NODE and _FLAM3H_DATA_PRM_MPIDX == -1:
                            ...
                            menu: list = [ 0, f"{FLAM3H_ICON_COPY_PASTE_INFO_WARN}  REMOVED: The marked iterator has been removed from node: {path}\n-> Mark an existing iterator instead.", 1, "" ]
                        else:
    ...
    def prm_paste_update_for_undo(self, node: hou.SopNode) -> tuple[hou.SopNode | None, int | None, bool]:
        ...
        It is for: hou.session.F3H_MARKED_ITERATOR_MP_IDX -> UNDO, so to speak -> prm: FLAM3H_DATA_PRM_MPIDX
        ...
        with hou.undos.disabler(): # type: ignore
            ...
            if mp_id_from is not None and from_FLAM3H_NODE is not None:
                ...
                if node == from_FLAM3H_NODE and self.exist_user_data(from_FLAM3H_NODE) is False:
                    for f3h in node.type().instances():
                        if f3h != node and self.exist_user_data(f3h):
                            ...
                            mp_id_from = hou.session.F3H_MARKED_ITERATOR_MP_IDX = self.get_user_data(f3h) # type: ignore
                ...
                # Mark, mark another node, Undo, Redo
                    ...
                    mp_id_from = hou.session.F3H_MARKED_ITERATOR_MP_IDX = self.get_user_data(node) # type: ignore
            ...
            # Mark, Clear, Mark, Undo
                ...
                if node == from_FLAM3H_NODE and self.exist_user_data(from_FLAM3H_NODE):
                    mp_id_from = hou.session.F3H_MARKED_ITERATOR_MP_IDX = self.get_user_data(from_FLAM3H_NODE) # type: ignore
        ...
        if node == from_FLAM3H_NODE: # type: ignore
            ...
            if mp_id_from is not None:
                ...
                if from_FLAM3H_NODE.parm(FLAM3H_DATA_PRM_MPIDX).eval() == -1:
                    _MSG: str = f"{node.name()}: {_MSG_UNMARKED}:  {idx}   Unmarked removed iterator -> {str(hou.session.F3H_MARKED_ITERATOR_MP_IDX)}" # type: ignore
                else:
    ...
    def prm_paste_CLICK(self, id: int) -> None:
        ...
        if node == hou.session.F3H_MARKED_ITERATOR_NODE: # type: ignore
            ...
            if hou.session.F3H_MARKED_ITERATOR_MP_IDX != id: # type: ignore
                ...
                _MSG: str = f"{self.node.name()}: iterator MARKED:  {str(hou.session.F3H_MARKED_ITERATOR_MP_IDX)}" # type: ignore
                flam3h_general_utils.set_status_msg(_MSG, 'IMP')
                flam3h_general_utils.flash_message(node, f"{str(hou.session.F3H_MARKED_ITERATOR_MP_IDX)}: MARKED") # type: ignore
    ...
    def prm_paste(self) -> None:
        ...
        if self.kwargs["ctrl"]:
            ...
            if self.exist_user_data(node) and int(self.get_user_data(node)) == idx and idx == hou.session.F3H_MARKED_ITERATOR_MP_IDX and node == hou.session.F3H_MARKED_ITERATOR_NODE: # type: ignore
    ...
    def prm_paste_FF_SHIFT(self) -> None:
        ...
        if from_FLAM3H_NODE_FF_CHECK is not None: # type: ignore
            if node == from_FLAM3H_NODE:
                ...
                _MSG: str = f"{node.name()}: This FF is Unmarked already. The marked FF is from node: {str(hou.session.F3H_MARKED_FF_NODE)}.FF" # type: ignore
                flam3h_general_utils.set_status_msg(_MSG, 'MSG')
        else:
            if isDELETED:
    ...
    def prm_paste_sel(self) -> None:
        ...
        try: mp_id_from: TA_M = hou.session.F3H_MARKED_ITERATOR_MP_IDX # type: ignore
    ...
    def prm_paste_sel_post_affine_FF(self) -> None:
        ...
        if kwargs['shift']:
            ...
            _MSG: str = f"FF PRE affine ALL values copied into the FF POST affine."
        flam3h_general_utils.set_status_msg(f"{self.node.name()}: {_MSG}", 'IMP')
    ...
    def prm_paste_sel_FF(self) -> None:
        ...
        try: from_FLAM3H_NODE_FF_CHECK: TA_M = hou.session.F3H_MARKED_FF_CHECK # type: ignore
    ...
    def auto_set_xaos(self) -> None:
        ...
        if len(_idx) > 1:
            ...
            if (len(s_history) > len(s_current)):
                ...
                try: flam3h_node_mp_id: TA_M = hou.session.F3H_MARKED_ITERATOR_MP_IDX # type: ignore
    ...
    def in_to_flam3h(self) -> None:
        ...
        if xml is not None and _xml_tree(xml).isvalidtree:
            ...
            if apo_data.xforms is not None:
                ...
                flam3h_general_utils.private_prm_set(node, PREFS_PVT_XF_VIZ_SOLO_MP_IDX, 0) # Reset mp index to Off value: 0(Zero)
                flam3h_general_utils.private_prm_set(node, PREFS_PVT_XF_FF_VIZ_SOLO, 0) # Turn Off FF xf viz solo mode
                flam3h_iterator_utils.destroy_userData(node, f"{FLAM3H_USER_DATA_PRX}_{FLAM3H_USER_DATA_XF_VIZ}")
                # BUILD XFVIZ if needed
                flam3h_general_utils.util_xf_viz_force_cook(node, self.kwargs)
...
* menu_out_contents_presets_data(self, node: hou.SopNode, xml_file_path: str, xml_is_file: bool) -> list:
