    ...
    if(!type) return _p*w;
        ...
        if(type<86){
            ...
            // 73 SPLITS ( parametric )
                ...
                vector2 splits;
                splits = GMP.splits[idx];
                V_SPLITS(p, _p, w, splits);
                return p; }
            // 74 STRIPES ( parametric )
            else if(type==74){
                vector2 stripes;
                stripes = GMP.stripes[idx];
