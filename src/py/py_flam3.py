...
reset_IN(self, mode: int=0) -> None:
    ...
    def in_load_render_stats_msg(preset_id: int, apo_data: in_flame_iter_data) -> str:
        ...
        cc_curves = []
        if apo_data.out_curve_overall[preset_id]!=OUT_XML_FLAME_RENDER_CURVE_DEFAULT and apo_data.out_curve_overall[preset_id] != OUT_XML_FLAME_RENDER_CURVE_DEFAULT_B: cc_curves.append('Overall')
        if apo_data.out_curve_red[preset_id]!=OUT_XML_FLAME_RENDER_CURVE_DEFAULT and apo_data.out_curve_red[preset_id]!=OUT_XML_FLAME_RENDER_CURVE_DEFAULT_B: cc_curves.append('Red')
        if apo_data.out_curve_green[preset_id]!=OUT_XML_FLAME_RENDER_CURVE_DEFAULT and apo_data.out_curve_green[preset_id]!=OUT_XML_FLAME_RENDER_CURVE_DEFAULT_B: cc_curves.append('Green')
        if apo_data.out_curve_blue[preset_id]!=OUT_XML_FLAME_RENDER_CURVE_DEFAULT and apo_data.out_curve_blue[preset_id]!=OUT_XML_FLAME_RENDER_CURVE_DEFAULT_B: cc_curves.append('Blue')
    ...
    def in_copy_cc_curves_stats_msg(kwargs: dict) -> None:
        ...
        if inisvalidfile and inisvalidpreset and not clipboard:
            ...
            if (cc_o.strip() == cc_r.strip() == cc_g.strip() == cc_b.strip() == OUT_XML_FLAME_RENDER_CURVE_DEFAULT) or (cc_o.strip() == cc_r.strip() == cc_g.strip() == cc_b.strip() == OUT_XML_FLAME_RENDER_CURVE_DEFAULT_B):
    ...
    def in_to_flam3h_set_palette(self, node: hou.SopNode, apo_data: in_flame_iter_data, _FLAM3H_INIT_DATA: tuple[Union[str, None], bool, int, str, bool, bool]) -> None:
        ...
        if apo_data.palette is not None:
            ...
            # Mark this as not a loaded palette preset
            node.setParms({CP_ISVALID_PRESET: 0}) # type: ignore
    ...
    def in_to_flam3h_toggles_and_msg(self, node: hou.SopNode, apo_data: in_flame_iter_data, _FLAM3H_INIT_DATA: tuple[Union[str, None], bool, int, str, bool, bool]) -> None:
        ...
        if clipboard:
            ...
            node.setParms({IN_CLIPBOARD_TOGGLE: 1}) # type: ignore
...
out_collect_var_section_names(node: hou.SopNode, var_section="VAR") -> Union[list[str], None]:
...
__out_flame_data_flam3h_toggle(self, toggle: bool) -> str:
    ...
    def out_render_curves_compare(node: hou.SopNode, mode: bool=False) -> bool:
        ...
        if not mode:
            if (cc_o == cc_r == cc_g == cc_b == OUT_XML_FLAME_RENDER_CURVE_DEFAULT) or (cc_o == cc_r == cc_g == cc_b == OUT_XML_FLAME_RENDER_CURVE_DEFAULT_B): return True
            else: return False
        else:
