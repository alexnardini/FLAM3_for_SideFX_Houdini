...
'''
    ...
    Tested on:  PYTHON v3.13.10 (H22.0)

    Title:      FLAM3H™USD H22.0. SideFX Houdini FLAM3: PYTHON
...
# FLAM3H™USD misc namespace collections
##########################################
##########################################
##########################################
##########################################
...
class f3h_web:
    ...
    '''
    # FLAM3H™
    F3H_KEY_SITE: Final = 'web'
    F3H_WEB_SITE: Final = 'https://www.alexnardini.net/'
    F3H_HEADING_SITE: Final = 'FLAM3H™ web'
    F3H_KEY_GIT: Final = 'git'
    F3H_WEB_GIT: Final = 'https://github.com/alexnardini/FLAM3_for_SideFX_Houdini'
    F3H_HEADING_GIT: Final = 'FLAM3H™ github'
    F3H_KEY_INSTA: Final = 'insta'
    F3H_WEB_INSTA: Final = 'https://www.instagram.com/alexnardini/'
    F3H_HEADING_INSTA: Final = 'FLAM3H™ instagram'
    F3H_KEY_YOUTUBE: Final = 'youtube'
    F3H_WEB_YOUTUBE: Final = 'https://www.youtube.com/@alexnardiniITALY/videos'
    F3H_HEADING_YOUTUBE: Final = 'FLAM3H™ youtube tutorials'
    
    # FLAM3
    F3_KEY_PAPER: Final = 'paper'
    F3_WEB_PAPER: Final = 'https://flam3.com/flame_draves.pdf'
    F3_HEADING_PAPER: Final = 'The Fractal Flame Algorithm(FLAM3) pdf'
    F3_KEY_GIT: Final = 'flam3git'
    F3_WEB_GIT: Final = 'https://github.com/scottdraves/flam3'
    F3_HEADING_GIT: Final = 'The Fractal Flame Algorithm(FLAM3) github'
    
    # FRACTORIUM
    FRACT_KEY_BIT: Final = 'fractbit'
    FRACT_WEB_BIT: Final = 'https://bitbucket.org/mfeemster/fractorium/src/master/'
    FRACT_HEADING_BIT: Final = 'Fractorium bitbucket'
    FRACT_KEY_GIT: Final = 'fractgit'
    FRACT_WEB_GIT: Final = 'https://github.com/mfeemster/fractorium/tree/master'
    FRACT_HEADING_GIT: Final = 'Fractorium github'
    FRACT_KEY_SITE: Final = 'fractweb'
    FRACT_WEB_SITE: Final = 'http://fractorium.com/'
    FRACT_HEADING_SITE: Final = 'Fractorium web'
...
@METHODS
    ...
    def setParms(node: hou.LopNode, parms_dict: dict, revertToDefaults: bool = False) -> None:
        ...
        Args:
            ...
            revertToDefaults(bool): If True, the parameters will be reverted to their default values instead of setting the new values.
...
* unpin_parameter_editor_with_f3husd_node(f3husd_node: hou.LopNode) -> None:
...
* flam3h_on_deleted(self) -> None:
    ...
    @staticmethod
    def flam3husd_compatible_h_versions_msg(msg: bool = True, ps_cls_about: bool = False) -> str:
        ...
        if len(__h_versions__) > 1:
            if __range_type__ is True: 
                ...
                if ps_cls_about:
                    _MSG_H_VERSIONS = f"H{flam3husd_scripts.flam3husd_h_versions_build_data(__h_versions__)} to H{flam3husd_scripts.flam3husd_h_versions_build_data(__h_versions__, True)}"
                else:
                    _MSG_H_VERSIONS = f"from H{flam3husd_scripts.flam3husd_h_versions_build_data(__h_versions__)} to H{flam3husd_scripts.flam3husd_h_versions_build_data(__h_versions__, True)}"
                ...
                if ps_cls_about:
                    ...
                    _MSG_H_VERSIONS = f"from H{flam3husd_scripts.flam3husd_h_versions_build_data(__h_versions__)} to H{flam3husd_scripts.flam3husd_h_versions_build_data(__h_versions__, True)} and up"
                    
        else:
            
            if __range_type__ is True:
                _MSG_H_VERSIONS = f"H{flam3husd_scripts.flam3husd_h_versions_build_data(__h_versions__)}"
    ...
    @staticmethod
    def flam3husd_compatible(h_version: int, kwargs: dict | None, msg: bool) -> bool:
        ...
        * def flam3h_compatible_range_open(kwargs: dict | None, msg: bool) -> bool:
        ...
        # we allow it to run anyway letting the user know that something can go wrong.
            ...
            if msg: flam3husd_scripts.flam3husd_compatible_h_versions_msg()
    ...
    def flam3husd_compatible_range_close(kwargs: dict | None, msg: bool) -> bool:
        ...
        * range_close -> mean FLAM3H™USD will run only on Houdini versions included inside: nodetype.hdaModule().__h_versions__
        ...
        return flam3husd_scripts.flam3husd_compatible(h_version, kwargs, msg)
    ...
    @staticmethod
    def unpin_parameter_editor_with_f3husd_node(f3husd_node: hou.LopNode) -> None:
        ...
        * def flam3h_on_deleted(self) -> None:
        ...
        Args:
            f3husd_node(hou.LopNode): The FLAM3H™USD node to check if its parameter are being pinned inside a Parameter Editor.
...
* flash_message(msg: str | None, timer: float = f3husd_tabs.DEFAULT_FLASH_MESSAGE_TIMER, img: str | None] = None) -> None:
...
* viewportParticleSize(self, reset_val: float | None = None, prm_name_size: str = f3husd_tabs.PREFS.PRM_VIEWPORT_PT_SIZE) -> None:
...
* flam3husd_display_help(self) -> None:
    ...
    def get_node_path(self, node_name: str) -> str | None:
        ...
        * f3husd_nodeNames.DEFAULT_OUT_BBOX_REFRAME
...
* flam3husd_web_run(self, key: str) -> None:
    ...
    def flam3husd_about_msg(self):
        ...
        flam3husd_author: str = f"AUTHOR: {__author__}"
        flam3husd_cvex_code: str = f"CODE: vex H{flam3husd_scripts.flam3husd_h_versions_build_data(flam3husd_general_utils.houdini_version(2), True)}"
        flam3husd_python_version: str = f"python {__py_version__}"
        flam3husd_houdini_version: str = f"VERSION: {__version__} - {__status__} :: ({__license__})"
        Implementation_build: str = f"{flam3husd_author}\n{flam3husd_houdini_version}\n{flam3husd_cvex_code}, {flam3husd_python_version}\n{__copyright__}"
        
        
        build: tuple[str, ...] = (Implementation_build, 
                                )
    ...
    def flam3husd_about_web_msg(self) -> None:
        ...
        # {prm_name: value, ...}
        parms_about_web: dict[str, str] = { f3husd_tabs.ABOUT.MSG_PRM_F3H_WEB: f3h_web.F3H_HEADING_SITE,
                                            f3husd_tabs.ABOUT.MSG_PRM_F3H_GIT: f3h_web.F3H_HEADING_GIT,
                                            f3husd_tabs.ABOUT.MSG_PRM_F3H_INSTA: f3h_web.F3H_HEADING_INSTA,
                                            f3husd_tabs.ABOUT.MSG_PRM_F3H_YOUTUBE: f3h_web.F3H_HEADING_YOUTUBE,
                                            f3husd_tabs.ABOUT.MSG_PRM_FLAM3_PDF: f3h_web.F3_HEADING_PAPER,
                                            f3husd_tabs.ABOUT.MSG_PRM_FLAM3_GIT: f3h_web.F3_HEADING_GIT,
                                            f3husd_tabs.ABOUT.MSG_PRM_FRACT_GITHUB: f3h_web.FRACT_HEADING_GIT,
                                            f3husd_tabs.ABOUT.MSG_PRM_FRACT_WEB: f3h_web.FRACT_HEADING_SITE
                                            }
        flam3husd_prm_utils.setParms(node, parms_about_web)
...
class pyside_master:
    ...
    class F3HUSD_msg_panel(QtWidgets.QWidget): # pyright: ignore -> this is becasue the import branches at the top
        ...
        def _load_svg_icon(self) -> None:
            ...
            try:
                ...
                assert self.SVG_ICON is not None
        ...
        def _build_ui(self) -> None:
            ...
            main_layout.setAlignment(QtCore.Qt.AlignmentFlag.AlignTop | QtCore.Qt.AlignmentFlag.AlignCenter)
            ...
            if not self.h_valid and not self.splash_screen:
                text: str = self.f3husd_node.parm(f3husd_tabs.ABOUT.MSG_PRM_ABOUT_ERROR).eval() if self.f3husd_node is not None else 'Error'
                self.svg_text_label.setText(f"Error\n{text}")
            self.svg_text_label.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
            font = QtGui.QFont(self.font_os)
            ...
            if self.LINKS:
                ...
                links_label.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
                ...
                links_label.setTextInteractionFlags(QtCore.Qt.TextInteractionFlag.TextBrowserInteraction)
        ...
        def _update_banner(self) -> None:
            if self.IMG_PIXMAP:
                try:
                    ...
                    scaled: QtGui.QPixmap = self.IMG_PIXMAP.scaled(w, h, QtCore.Qt.AspectRatioMode.KeepAspectRatioByExpanding, QtCore.Qt.TransformationMode.SmoothTransformation)
        ...
        def _position_credit_label(self) -> None:
            if hasattr(self, "credit_label"):
                ...
                x: int = int(self.banner_container.width() - self.credit_label.width() - margin)
                y: int = int(self.banner_container.height() - self.credit_label.height() - margin)
