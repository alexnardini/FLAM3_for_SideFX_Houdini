...
- **GPU performance**<br>FLAM3H™ GPU mode (_the default in H21 and up_) can be hundreds of times faster than the CPU(_Cvex_) mode depending on the system' GPU device being used. Tests using an Nvidia RTX 4090 device reveal throughput ranging from ~50 to ~100 billion iterations per second. The tests were performed with a variety of Flame presets over 500 millions points at 1024 iterations each (_the GPU mode iterations default_).
