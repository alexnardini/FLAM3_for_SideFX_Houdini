...
## [<ins>v2.0.61 indie</ins>](https://github.com/alexnardini/FLAM3_for_SideFX_Houdini/releases/tag/v2.0.61)  ##
...
#### FLAM3H™USD v0.2.64 indie -> H22.0 UP</br>FLAM3H™USD v0.2.60 indie -> H21.0</br>FLAM3H™USD v0.2.56 indie -> H20.5</br>FLAM3H™USD v0.2.50 indie -> H19.0 to H20 ####
...
## [<ins>v2.0.52 indie</ins>](https://github.com/alexnardini/FLAM3_for_SideFX_Houdini/releases/tag/v2.0.52)  ##
...
- **New H21.0 UP Only**: Added the ability to enter the Camera SENSOR VIZ mode from the SYS tab TAG icons.
- **Fix**: When the loaded IN Flame preset is being modified on disk, one of the entry was being marked with a double asterisk(**) instead of one(*). It is now fixed.
- **Fix**: The OpenCL **sample_cdf_binary()** function had an error as a regression from one of the preview releases. It is now fixed.
...
- **New H21.0 UP ONLY**: It is now possible to switch between **GPU** and **CPU** modes from the GLOBAL tab iterations' icons, read the HDA documentation or their tooltips to know more.
- **Fix**: When saving a Flame out while using the **Blur** variation inside an iterator or FF **PRE** section and loading it back in, FLAM3H™ was setting the standard **Pre_blur** slot instead. It is now fixed.
- **Small fixes/improvements as part of an ongoing polishing pass.**
- **Updated HDA documentation.**
...
- **OpenCL variations BWRAPS, MODULUS, CPOW, GLYNNIA, TWINTRIAN, CROP and CELL have been updated/improved. H20.5 UP ONLY**
...
#### FLAM3H™USD v0.2.58 indie -> H21.0 UP</br>FLAM3H™USD v0.2.54 indie -> H20.5</br>FLAM3H™USD v0.2.49 indie -> H19.0 to H20 ####
...
#### FLAM3H™ v2.0.23 indie -> H21.0 UP</br>FLAM3H™ v2.0.19 indie -> H20.5</br>FLAM3H™ v1.9.81 indie -> H19.0 to H20 ####
...
- **Fix H21 UP ONLY**: Updated a few UI icons.
- **Updated HDA documentation H20.5 and H21.0**: Finally got to update some parts of the documentation since the introduction of the GPU mode.
...
- **Fix**: Safer OpenCL memory layout for Xaos data and Shader data. This should prevent rare crashes, if any, on some GPU devices and drivers.
...
- **Fix**: Updated FLAM3H™ content references to python class attributes and global variable names.
- **Small fixes/improvements as part of an ongoing polishing pass.**
- **Updated HDA documentation.**
- **Updated GIT documentation.**
...
- **Fix**: FLAM3H™ can now deal with locked parameters.
- **Updated HDA documentation.**
...
- **New H20.5 to H21.0 UP ONLY**: Added splash screen on first node instance creation and about box as well.
- **Small fixes/improvements as part of an ongoing polishing pass**
...
- **Fix H21.0 UP ONLY**: Fixed a few small bugs introduced with the preview release's python module updates (regressions).
...
- **Fix H21.0 UP ONLY**: Improved a little the UI performance.
...
#### FLAM3H™ v1.9.62 indie -> H21.0 UP</br>FLAM3H™ v1.9.55 indie -> H19.0 to H20.5 ####
...
- **Fix**: Copy/Paste FLAM3H™ nodes between different Houdini sessions could trigger an AttributeError when running the Zero opacities checks inside the OnLoaded script definition.(_A regression from preview versions_.)
...
#### FLAM3H™ v1.9.50 indie -> H21.0 UP</br>FLAM3H™ v1.9.44 indie -> H19.0 to H20.5 ####
...
- **New**: Preference density value parameter now have an action button icon to be reset to its default value.
- **Updated HDAs documentation.**
...
- **Fix**: pre_blur and mobius in the same iterator causing a mismatch of value when loading a flame.
- **Fix**: Mismatch of variations when loading flames in some cases.
- **Fix**: Saving out a palette to a filepath not existing yet was erroring out. 
- **Fix**: Post affine checkboxes now uses the header_toggle builtin tag, much more cleaner.
- **Fix**: When iterator's count is ZERO the density value will now reset back to default (500k).
- **Fix**: When iterator's count is ZERO, the node descriptive message will now clear up.
- **Fix**: When selecting a non valid file location in the IN flame file parameter the infos and IN render properties Tabs will now hide.
- **Fix**: Loading an hip file with a FLAM3H™ node in it with a JSON, IN or OUT file path no longer valid will now properly hide unnecessary Tabs and print out messages accordingly.
- **Fix**: Palette and IN presets now kept when loading back a hip file with those menus set.
- **Fix**: Force update about tab on load to pick up changes made on the moment.
- **Fix**: Little IN Tab UI redesign.
- **Fix**: Little python code improvements.
- **Fix**: The IN Tab parameter option "Use Fractorium color speed" has been hidden.
...
- **Fix**: corrected pre variations behavior.
- **Fix**: iterator's Weight not being set on flame load when its value was Zero.
- **Fix**: "xaos: " command string is now smarter about what you type in.
- **Fix**: iterations on flame load not always using its default values.
- **Fix**: pre_blur variation not always being set on flame load.
- **Fix**: out of bounds palette's color values are now taken care of.
- **Fix**: Auger variation.
- **Fix**: Fan variation.
- **Fix**: iterators are now created with a default name.
- **Fix**: the "xaos:" keyword is now present as a default inside each iterator.
- **Fix**: improved OTL documentation.
- **Fix**: parameters tooltips now showing more descriptive and generous descriptions.
...
- **Fix**: flame files created with older versions of Apophysis
  didn’t load due to missing key names in the file.
  Now it work and the missing keys are set to a default value.
- **Fix**: palette colors from flame load are now more vibrant.
- **Fix**: Palette HSV values now default to: (1, 1, 1) making possible to hue backward.
- **Fix**: Juliascope variation distance parameter not being set on flame load.
...
- **Fix**: Sech.
- **Fix**: Csc.
- **Fix**: wedge, wedge_julia, wedge_sph. They had their parameters order mixed up.
